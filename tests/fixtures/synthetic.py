...
def write_synthetic_parquets(out_dir: Path) -> None:
    ...
    pd.DataFrame(
        {"condition_id": [1, 2, 3, 4, 5],
         "name": ["Near Mint", "Lightly Played", "Moderately Played", "Heavily Played", "Damaged"]}
    ).to_parquet(out_dir / "conditions.parquet")

    pd.DataFrame(
        {"printing_id": [1, 2], "name": ["Normal", "Foil"]}
    ).to_parquet(out_dir / "printings.parquet")

    pd.DataFrame(
        {"language_id": [1, 2, 3, 7],
         "name": ["English", "Chinese (S)", "Chinese (T)", "Japanese"]}
    ).to_parquet(out_dir / "languages.parquet")

    pd.DataFrame(
        {"rarity_id": [1, 2, 3, 4],
         "name": ["Common", "Uncommon", "Rare", "Mythic Rare"]}
    ...
    pd.DataFrame(
        {"product_id":      [1001, 1002, 1003, 2001, 9999],
         ...
         "name":            ["Alpha Card 1", "Alpha Card 2", "Alpha Card 3", "Beta Card 1", "Alpha Sealed Box"],
         "clean_name":      ["Alpha Card 1", "Alpha Card 2", "Alpha Card 3", "Beta Card 1", "Alpha Sealed Box"],
         "image_url":       [f"https://tcgplayer-cdn.tcgplayer.com/product/{pid}_200w.jpg"
                             for pid in [1001, 1002, 1003, 2001, 9999]],
         "url":             [f"https://www.tcgplayer.com/product/{pid}/" for pid in [1001, 1002, 1003, 2001, 9999]],
    ...
    for pid in [1001, 1002, 1003, 2001]:
        for printing_id in [1, 2]:
            for condition_id in [1, 2]:
                skus.append({
                    "sku_id": sku_id, "product_id": pid,
                    "language_id": 1, "printing_id": printing_id, "condition_id": condition_id,
